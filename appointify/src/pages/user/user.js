...
const UserPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchOccupied = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:5000/user/occupied/${professionalData._id}`, {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
        ...
        console.error("Error fetching occupied:", error);
        setOccupied([]);
      }
    };
    ...
    <div>
      ...
      <div className="profile-container">
        <div className="time-slots-container">
          <h3>Occupied Time Slots</h3>
          {occupied.map((times, index) => (
            <div key={index} className="time-slot">
              <span>{formatDateTime(times.startTime)}</span> - <span>{formatDateTime(times.endTime)}</span>
            </div>
          ))}
        </div>
